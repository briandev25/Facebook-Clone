  ...
  return (
    <div className='flex justify-between items-center sticky top-0 z-50 shadow-md p-2 xl:px-5 bg-white'>
        ...
        <div className='flex items-center'>
            <div onClick={() => signOut()} className='hidden xl:flex items-center hover:bg-gray-300 p-2 rounded-full cursor-pointer '>
                {/* Profile Image Here */}
                 <Image className=' rounded-full cursor-pointer' alt='' src={session.user.image} width={30} height={30} layout='fixed'  />
                 <p className=' font-semibold text-sm px-3 whitespace-nowrap'>{session.user.name}</p>
            </div>
